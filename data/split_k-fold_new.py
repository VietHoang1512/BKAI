...
for label in intent_label:
    label_data_dict[label] = [(x, y, z) for x, y, z in zip(new_label, new_seqin, new_seqout) if x == label]
...
def split_fold(n_fold=5):
    if not os.path.exists('./%d-folds-new/' % (n_fold)):
        os.makedirs('./%d-folds-new/' % (n_fold))
    else:
        print("Have splited with %d folds" % (n_fold))
        return
    for fold in range(n_fold):
        if not os.path.exists('./%d-folds-new/fold-%d' % (n_fold, fold + 1)):
            os.makedirs('./%d-folds-new/fold-%d' % (n_fold, fold + 1))
        if not os.path.exists('./%d-folds-new/fold-%d/dev/' % (n_fold, fold + 1)):
            os.makedirs('./%d-folds-new/fold-%d/dev/' % (n_fold, fold + 1))
        if not os.path.exists('./%d-folds-new/fold-%d/train/' % (n_fold, fold + 1)):
            os.makedirs('./%d-folds-new/fold-%d/train/' % (n_fold, fold + 1))
    ...
    for key, value in fold_dict.items():
        ...
        with open('./%d-folds-new/fold-%d/dev/intent_label.txt' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in intent_label:
                f.write(label + '\n')
        with open('./%d-folds-new/fold-%d/dev/slot_label.txt' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in slot_label:
                f.write(label + '\n')
        with open('./%d-folds-new/fold-%d/dev/label' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in dev_label:
                f.write(label + '\n')
        with open('./%d-folds-new/fold-%d/dev/seq.in' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for seqin in dev_seqin:
                f.write(seqin + '\n')
        with open('./%d-folds-new/fold-%d/dev/seq.out' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for seqout in dev_seqout:
                f.write(seqout + '\n')
        with open('./%d-folds-new/fold-%d/train/intent_label.txt' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in intent_label:
                f.write(label + '\n')
        with open('./%d-folds-new/fold-%d/train/slot_label.txt' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in slot_label:
                f.write(label + '\n')
        with open('./%d-folds-new/fold-%d/train/label' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in train_label:
                f.write(label + '\n')
        with open('./%d-folds-new/fold-%d/train/seq.in' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for seqin in train_seqin:
                f.write(seqin + '\n')
        with open('./%d-folds-new/fold-%d/train/seq.out' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for seqout in train_seqout:
                f.write(seqout + '\n')
        os.system("cp intent_label.txt ./%d-folds-new/fold-%d/intent_label.txt" %  (n_fold, key + 1))
        os.system("cp slot_label.txt ./%d-folds-new/fold-%d/slot_label.txt" % ( n_fold, key + 1))
        os.system("cp -r test ./%d-folds-new/fold-%d/" % ( n_fold, key + 1))
