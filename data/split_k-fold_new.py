...
def split_fold(n_fold=5):
    ...
    for key, value in fold_dict.items():
        ...
        with open('./%d-folds/fold-%d/dev/intent_label.txt' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in intent_label:
                f.write(label + '\n')
        with open('./%d-folds/fold-%d/dev/slot_label.txt' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in slot_label:
                f.write(label + '\n')
        with open('./%d-folds/fold-%d/dev/label' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in dev_label:
                f.write(label + '\n')
        with open('./%d-folds/fold-%d/dev/seq.in' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for seqin in dev_seqin:
                f.write(seqin + '\n')
        with open('./%d-folds/fold-%d/dev/seq.out' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for seqout in dev_seqout:
                f.write(seqout + '\n')
        with open('./%d-folds/fold-%d/train/intent_label.txt' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in intent_label:
                f.write(label + '\n')
        with open('./%d-folds/fold-%d/train/slot_label.txt' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in slot_label:
                f.write(label + '\n')
        with open('./%d-folds/fold-%d/train/label' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for label in train_label:
                f.write(label + '\n')
        with open('./%d-folds/fold-%d/train/seq.in' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for seqin in train_seqin:
                f.write(seqin + '\n')
        with open('./%d-folds/fold-%d/train/seq.out' % (n_fold, key + 1), 'w', encoding='utf-8') as f:
            for seqout in train_seqout:
                f.write(seqout + '\n')
        os.system("cp intent_label.txt ./%d-folds/fold-%d/intent_label.txt" %  (n_fold, key + 1))
        os.system("cp slot_label.txt ./%d-folds/fold-%d/slot_label.txt" % ( n_fold, key + 1))
